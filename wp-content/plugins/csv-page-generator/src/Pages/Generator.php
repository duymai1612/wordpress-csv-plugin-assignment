...
class Generator {
	...
	public function create_page( array $page_data ) {
		...
		$this->logger->debug( 'Creating page', array(
		...
		$this->logger->info( 'Page created successfully', array(
			'page_id' => $page_id,
			'title'   => $prepared_data['post_title'],
	...
	private function handle_featured_image( $page_id, $image_url ) {
		try {
			...
			if ( $existing_attachment ) {
				...
				$this->logger->debug( 'Used existing attachment for featured image', array(
					'page_id'       => $page_id,
			...
			if ( $attachment_id ) {
				...
				$this->logger->info( 'Featured image set successfully', array(
					'page_id'       => $page_id,
	...
	public function bulk_create_pages( array $pages_data, array $options = array() ) {
		...
		$this->logger->info( 'Bulk page creation completed', array(
	...
	public function delete_import_pages( $import_id ) {
		...
		$this->logger->info( 'Import pages deletion completed', array(
			'import_id'     => $import_id,
			'total_pages'   => $results['total'],
